...
app.get('/api/damage/claim-types', (req, res) => {
    res.json({
        success: true,
        claimTypes: [
            {
...
app.post('/api/damage/analyze', upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
...
// Combined ZK verification + damage detection
app.post('/api/damage/verify-and-analyze', upload.single('image'), async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
        ...
        const result = {
            ...
            verdict: {
                ...
                message: isValidClaim ?
                    `✅ VALID CLAIM: Authentic photo shows ${damageResult.verdict.severity} damage` :
